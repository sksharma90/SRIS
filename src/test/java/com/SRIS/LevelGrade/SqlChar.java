...
public class SqlChar extends BaseTest implements SRIS_TestData
{
	...
	public void CkeckSqlCharForGradeName()throws InterruptedException
	{
		...
		if (constainsSpecial == true) {
			AutoConstant.fileone.setCellData(LevelGrade_Sheet, Column_name, 12, "Fail");
			System.out.println("Fail");
		} 
		else {
			AutoConstant.fileone.setCellData(LevelGrade_Sheet, Column_name, 12, "Pass");
			System.out.println("Pass");
		}
	}
	
	@Test(priority = 3)
	public void CkeckSqlCharForGradeNameDescription()throws InterruptedException
	{
		SRIS_MasterModule masterdata = new SRIS_MasterModule(driver);
		Thread.sleep(700);
		masterdata.GradeNameDescriptionClearMethod();
		Thread.sleep(700);
		masterdata.GradeNameDescriptionSqlCharMethod();
		Thread.sleep(700);
		...
		boolean constainsSpecial = matcher.find();
		if (constainsSpecial == true) {
			AutoConstant.fileone.setCellData(LevelGrade_Sheet, Column_name, 13, "Fail");
			System.out.println("Fail");
		} 
		else {
			AutoConstant.fileone.setCellData(LevelGrade_Sheet, Column_name, 13, "Pass");
			System.out.println("Pass");
		}
	}
