...
public class SqlChar extends BaseTest implements SRIS_TestData
{
	...
	@Test(priority = 2)
	public void CkeckSqlCharForMaritalStatusName()throws InterruptedException
	{
		SRIS_MasterModule masterdata = new SRIS_MasterModule(driver);
		Thread.sleep(700);
		masterdata.MaritalStatusNameClearMethod();
		Thread.sleep(700);
		masterdata.MaritalStatusNameSqlCharMethod();
		Thread.sleep(700);
		...
		if (constainsSpecial == true) {
			AutoConstant.fileone.setCellData(MaritalStatusMaster_Sheet, Column_name, 8, "Fail");
			System.out.println("Fail");
		} 
		else {
			AutoConstant.fileone.setCellData(MaritalStatusMaster_Sheet, Column_name, 8, "Pass");
